...
/**
 * Latest Auctions Widget
 *
 * @class UWA_Widget_Latest_Auctions
 ...
 *
 */
class UWA_Widget_Latest_Auctions extends WP_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce widget_uwa_latest_auctions';
		$this->widget_description = __( 'Display a list of Latest Auctions from your store.', 'woo_ua' );
		$this->widget_id          = 'woocommerce_uwa_live_auctions';
		$this->widget_name        = __( 'UWA Latest Auctions', 'woo_ua' );
		
		/* Widget settings. */
		$uwa_widget_arg = array( 'classname' => $this->widget_cssclass, 'description' => $this->widget_description );
		parent::__construct('uwa_latest_auctions', $this->widget_name, $uwa_widget_arg);		
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_uwa_latest_auctions', 'widget');
	}
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('widget_uwa_latest_auctions', 'widget');		
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Latest Auctions', 'woo_ua' ) : $instance['title'], $instance, $this->id_base);
		...
		if ( $uwa_query->have_posts() ) {
			...
			<li><?php _e('Latest auctions not found', 'woo_ua');?></li>			
		<?php }
		echo '</ul>';
		echo $after_widget;	
		wp_reset_postdata();		
		...
		echo $content;
		wp_cache_set('widget_uwa_latest_auctions', $cache, 'widget');		
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_uwa_latest_auctions']) ) delete_option('widget_uwa_latest_auctions');
