 ...
 *
 * @class UWA_Widget_Live_Auctions
 ...
 *
 */
class UWA_Widget_Live_Auctions extends WP_Widget {
	
	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce widget_uwa_live_auctions';
		$this->widget_description = __( 'Display a list of Live Auctions from your store.', 'woo_ua' );
		$this->widget_id          = 'woocommerce_uwa_live_auctions';
		$this->widget_name        = __( 'UWA Live Auctions', 'woo_ua' );
		
		/* Widget settings. */
		$uwa_widget_arg = array( 'classname' => $this->widget_cssclass, 'description' => $this->widget_description );
		parent::__construct('uwa_live_auctions', $this->widget_name, $uwa_widget_arg);		
	...
	function flush_widget_cache() {		
		wp_cache_delete('widget_uwa_live_auctions', 'widget');
	}
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('widget_uwa_live_auctions', 'widget');		
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Live Auctions', 'woo_ua' ) : $instance['title'], $instance, $this->id_base);
        ...
        $query_args['meta_query'][] = array('key' => 'woo_ua_auction_has_started','compare' =>'==', 'value'=>'1');
		...
		$uwa_hide_time = empty( $instance['uwa_hide_time'] ) ? 0 : 1;
			...
			<li><?php _e('Live auction not found', 'woo_ua');?></li>			
		<?php }
		echo '</ul>';		
		echo $after_widget;
		wp_reset_postdata();		
		...
		echo $content;
		wp_cache_set('widget_uwa_live_auctions', $cache, 'widget');		
	}
	...
	*/
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_uwa_live_auctions']) ) delete_option('widget_uwa_live_auctions');
