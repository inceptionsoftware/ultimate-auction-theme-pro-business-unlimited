...
/**
 * Recently View Auctions Widget
 *
 * @class UWA_Widget_Recently_View_Auctions
 ...
 *
 */
class UWA_Widget_Recently_View_Auctions extends WP_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce widget_uwa_recently_view_auctions';
		$this->widget_description = __( 'Display a list of Recently View Auctions from your store.', 'woo_ua' );
		$this->widget_id          = 'woocommerce_uwa_live_auctions';
		$this->widget_name        = __( 'UWA Recently View Auctions', 'woo_ua' );
		
		/* Widget settings. */
		$uwa_widget_arg = array( 'classname' => $this->widget_cssclass, 'description' => $this->widget_description );
		parent::__construct('uwa_recently_view_auctions', $this->widget_name, $uwa_widget_arg);		
	...
	function flush_widget_cache() {		
		wp_cache_delete('widget_uwa_recently_view_auctions', 'widget');
	}
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('widget_uwa_recently_view_auctions', 'widget');		
		...
		$viewed_auction = ! empty( $_COOKIE['uwa_recently_viewed_auctions'] ) ? (array) explode( '|', $_COOKIE['uwa_recently_viewed_auctions'] ) : array();
		$viewed_auction = array_filter( array_map( 'absint', $viewed_auction ) );

		
		if ( empty( $viewed_auction ) )
			return;

		ob_start();
		extract($args);
		
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recently View Auctions', 'woo_ua' ) : $instance['title'], $instance, $this->id_base);
		...
		$query_args = array('posts_per_page' => $number, 'no_found_rows' => 1, 'post_status' => 'publish', 'post_type' => 'product', 'post__in' => $viewed_auction, 'orderby' => 'rand');
		...
		if ( $uwa_query->have_posts() ) {
			while ( $uwa_query->have_posts()) {
				...
				<div class="uwa_auction_product_countdown" data-time="<?php echo $remaning_time;?>" data-auctionid="<?php echo $product_id;?>" data-format="<?php echo get_option( 'woo_ua_auctions_countdown_format' );?>" data-timezonetype="<?php echo $uwa_time_zone['timezone_type']; ?>" data-zone="<?php echo $uwa_time_zone['timezone']; ?>"></div>
				...
				<div class="uwa_auction_product_countdown" data-time="<?php echo $starting_time;?>" data-auctionid="<?php echo $product_id;?>" data-format="<?php echo get_option( 'woo_ua_auctions_countdown_format' );?>" data-timezonetype="<?php echo $uwa_time_zone['timezone_type']; ?>" data-zone="<?php echo $uwa_time_zone['timezone']; ?>"></div>
				  <?php } ?>
				<?php } ?>
			...
			<li><?php _e('Recently View Auctions not found', 'woo_ua');?></li>
		<?php }
		echo '</ul>';
		echo $after_widget;		
		wp_reset_postdata();		
		...
		echo $content;
		wp_cache_set('widget_uwa_recently_view_auctions', $cache, 'widget');		
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_uwa_recently_view_auctions']) ) delete_option('widget_uwa_recently_view_auctions');
