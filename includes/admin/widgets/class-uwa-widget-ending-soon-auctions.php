 ...
 *
 */
class UWA_Widget_Ending_Soon_Auctions extends WP_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce widget_uwa_ending_soon_auctions';
		$this->widget_description = __( 'Display a list of Ending Soon Auctions from your store', 
			'woo_ua' );
		$this->widget_id          = 'woocommerce_uwa_uwa_ending_soon_auctions';
		$this->widget_name        = __( 'UWA Ending Soon Auctions', 'woo_ua' );
		
		/* Widget settings. */
		$uwa_widget_arg = array( 'classname' => $this->widget_cssclass, 'description' => $this->widget_description );
		parent::__construct('uwa_ending_soon_auctions', $this->widget_name, $uwa_widget_arg);
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_uwa_ending_soon_auctions', 'widget');
	}	
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('widget_uwa_ending_soon_auctions', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Ending Soon Auctions', 'woo_ua' ) : $instance['title'], $instance, $this->id_base);
		...
		if ( $uwa_query->have_posts() ) {
			...
			while ( $uwa_query->have_posts()) {
				...
				$auction_image = ( has_post_thumbnail() ? get_the_post_thumbnail( $uwa_query->post->ID, 
					'shop_thumbnail' ) : wc_placeholder_img ( 'shop_thumbnail' ) );
			...
			<li><?php _e('Ending Soon Auction not found', 'woo_ua');?></li>
		...
		echo $content;
		wp_cache_set('widget_uwa_ending_soon_auctions', $cache, 'widget');		
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_uwa_ending_soon_auctions']) ) 
			delete_option('widget_uwa_ending_soon_auctions');
